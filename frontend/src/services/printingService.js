...
import { formatarDataBr, mascararCpf } from "../utils/masks";
...
export function htmlDiagnostico(d) {
  ...
  <main class="sheet">
    ...
    <section class="patient-grid">
      <div>
        ...
        <div class="line"><span class="label">CPF:</span> ${esc(mascararCpf(d.patientCpf || ""))}</div>
      ...
      <div>
        ...
        <div class="line"><span class="label">Data do diagnóstico:</span> ${esc(formatarDataBr(d.createdAt))}</div>
...
export function htmlLinhaTempo(timeline) {
  return baseHtml(`Linha do tempo: ${timeline.name}`, `
    <p class="muted">Histórico clínico agrupado em ordem cronológica.</p>
    ${(timeline.diagnoses || []).map((d) => `<div class="box"><strong>${esc(formatarDataBr(d.createdAt))} — ${esc(d.title)}</strong><br>CID ${esc(d.cid)}<br><span class="muted">${esc(d.doctorName)} · ${esc(d.unitName)}</span><p>${esc(d.description)}</p></div>`).join("") || "<p>Nenhum diagnóstico vinculado.</p>"}
  `);
}
...
export async function gerarPdf(html, filename = "saude-app-documento.pdf") {
  if (Platform.OS === "web" && typeof window !== "undefined") {
